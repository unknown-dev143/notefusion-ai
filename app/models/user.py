...
class User(Base):
    ...
    avatar_url = Column(String(500), nullable=True)   # profile picture from Google
    google_id = Column(String(128), unique=True, index=True, nullable=True)  # Google sub claim
    ...
    # Scholar Tokens (purchased via Stripe)
...
def hash_password_before_update(mapper, connection, target):
    ...
    history = get_history(target, "hashed_password")
    if history.added and not history.added[0].startswith("$"):
        target.hashed_password = pwd_context.hash(history.added[0])
