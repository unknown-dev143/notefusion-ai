...
Revision ID: 2025_08_11_0956_add_subscription_models
Rev: 
Create Date: 2025-08-11 09:56:00.000000
...
revision = '2025_08_11_0956_add_subscription_models'
down_revision = None
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('tier', sa.Enum('free', 'pro', 'business', 'admin', name='subscriptiontier'), 
                 nullable=False, server_default='free'),
        sa.Column('status', sa.Enum('active', 'canceled', 'past_due', 'unpaid', 'trialing', 
                                  'incomplete', 'incomplete_expired', 'paused', name='subscriptionstatus'), 
    ...
    op.create_index(op.f('ix_invoices_payment_intent_id'), 'invoices', ['payment_intent_id'], unique=False)
