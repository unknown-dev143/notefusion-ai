# Stop any running Python processes
Write-Host "Stopping any running Python processes..."
Get-Process python -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue

# Set environment variables
$env:APP_NAME = "NoteFusion"
$env:APP_ENV = "development"
$env:DEBUG = "True"
$env:SECRET_KEY = "your-secret-key-here"
$env:ALGORITHM = "HS256"
$env:ACCESS_TOKEN_EXPIRE_MINUTES = "30"
$env:DATABASE_URL = "sqlite:///./notefusion_new.db"
$env:BACKEND_CORS_ORIGINS = '["http://localhost:3000","http://127.0.0.1:3000"]'

# Print environment variables for verification
Write-Host "`nEnvironment variables set:"
Write-Host "------------------------"
Get-ChildItem Env: | Where-Object { $_.Name -like "APP_*" -or $_.Name -like "DATABASE_*" -or $_.Name -like "SECRET_*" -or $_.Name -like "BACKEND_*" } | Format-Table -AutoSize

# Remove existing database file if it exists
$dbFile = "notefusion_new.db"
if (Test-Path $dbFile) {
    Write-Host "`nRemoving existing database file: $dbFile"
    Remove-Item $dbFile -Force
}

# Create a new database
Write-Host "`nCreating a new database..."
python -c "
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker
from app.models import Base
engine = create_engine('sqlite:///./notefusion_new.db')
Base.metadata.create_all(bind=engine)
print('Database created successfully at notefusion_new.db')
"

# Start the server
Write-Host "`nStarting the server..."
python -m uvicorn start_server:app --reload --log-level debug
