...
def test_list_tasks(client, auth_headers):
    ...
    assert len(all_tasks) >= len(tasks)  # There might be other tasks from other tests
...
def test_search_tasks(client, auth_headers):
    ...
    assert len(found_tasks) >= 1  # Should find tasks with "find" in title or description
...
def test_task_status_workflow(client, auth_headers):
    """Test the complete task status workflow."""
    # Create a new task (default status should be 'pending')
    ...
    update_response = client.patch(
        f"/api/v1/tasks/{task_id}/status/in_progress",
        headers=auth_headers
    )
    ...
    # Update status back to 'in_progress' (should clear completed_at)
    update_response = client.patch(
        f"/api/v1/tasks/{task_id}/status/in_progress",
        headers=auth_headers
    )
    ...
    response = client.patch(
        f"/api/v1/tasks/{task_id}/status/invalid_status",
        headers=auth_headers
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_validation_errors(client, auth_headers):
    ...
    invalid_tasks = [
        ...
        {"title": "Test", "status": "invalid_status"},  # Invalid status
        {"title": "Test", "priority": "invalid_priority"},  # Invalid priority
        {"title": "Test", "tags": ["a" * 101]},  # Tag too long
    ]
